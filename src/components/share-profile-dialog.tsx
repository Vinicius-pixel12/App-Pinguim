...
export function ShareProfileDialog({ open, onOpenChange, username, displayName }: Props) {
  const [copied, setCopied] = useState(false);

  const url = useMemo(() => {
    const origin = typeof window !== "undefined" ? window.location.origin : "";
    return `${origin}/perfil/${username.replace(/^@/, "")}`;
  }, [username]);

  const text = `Confira o perfil de ${displayName ?? "@" + username} no Pinguim`;
  ...
  const targets = [
    ...
    { name: "E-mail", icon: Mail, href: `mailto:?subject=${enc("Perfil no Pinguim")}&body=${enc(`${text}\n\n${url}`)}`, color: "text-muted-foreground" },
  ];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>Envie o link do perfil para onde quiser.</DialogDescription>
        </DialogHeader>
